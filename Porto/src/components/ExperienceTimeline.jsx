...
const TimelineItem = ({ data, index }) => {
  ...
  return (
    <div
      className={`relative flex flex-col sm:flex-row items-center transition-all duration-1000
        ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}
        ${index % 2 === 0 ? "sm:flex-row" : "sm:flex-row-reverse"}
      `}
    >
      {/* Timeline line */}
      <div className="hidden sm:block absolute left-1/2 w-px h-full bg-blue-500/30" />
          ...
          {data.skills && (
            <div className="flex flex-wrap gap-2 mt-4">
              {data.skills.map((skill) => (
                <span
                  key={skill}
                  className="px-3 py-1 text-xs sm:text-sm bg-blue-500/20 text-blue-300 rounded-full"
                >
                  {skill}
                </span>
              ))}
...
const ExperienceTimeline = () => {
  ...
  return (
    <section className="min-h-screen bg-gray-900 py-16 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-3xl sm:text-5xl font-bold text-center mb-12 text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-600">
          Experience & Education
        </h2>
